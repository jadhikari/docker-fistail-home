...
class Target(TimeStampedUserModel):
    ...
    target_to = models.ForeignKey(User, on_delete=models.CASCADE, related_name='assigned_targets', help_text="User who is assigned this target")

    # Super user who assigned the target
    assigned_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='targets_assigned', help_text="Super user who assigned this target")

    # Target amount (monthly)
    target_amount = models.DecimalField(max_digits=12, decimal_places=2, validators=[MinValueValidator(Decimal('0.01'))], help_text="Monthly target amount in Japanese Yen")
    ...
    target_month = models.PositiveIntegerField(choices=MONTH_CHOICES, help_text="Target month (1-12)")
    ...
    description = models.TextField(blank=True, null=True, help_text="Additional notes about the target")
...
class RentalContract(TimeStampedUserModel):
    ...
    target_to = models.ForeignKey(Target, on_delete=models.CASCADE, related_name='rental_contracts', help_text="Target this rental contract belongs to", null=True, blank=True)  # Temporarily allow null for existing records

    # Customer Information
    customer_name = models.CharField(max_length=255, help_text="Name of the customer/tenant")
    customer_number = models.CharField(max_length=11, help_text="Customer contact number (maximum 11 digits)")
    ...
    contract_type = models.CharField(max_length=20, choices=CONTRACT_TYPE_CHOICES, help_text="Type of rental contract")
    cancellation_notice_period = models.CharField(max_length=255, help_text="Notice period required for cancellation")
    cancellation_period = models.CharField(max_length=255, help_text="Cancellation period details")
    cancellation_charge = models.CharField(max_length=255, help_text="Charges applicable on cancellation")
    deposit_fee = models.CharField(max_length=255, help_text="Deposit fee amount and terms")
    cleaning_charge = models.BooleanField(default=False, help_text="Whether cleaning charge is applicable")

    # Emergency Contact
    emergency_contact_person = models.CharField(max_length=255, help_text="Emergency contact person name")
    emergency_phone = models.CharField(max_length=11, help_text="Emergency contact phone number (maximum 11 digits)")

    # Additional Information
    renew_fee = models.CharField(max_length=255, help_text="Renewal fee amount and terms")
    living_num_people = models.PositiveIntegerField(help_text="Number of people living in the property")
    rent_payment_date = models.CharField(max_length=255, help_text="Rent payment due date")

    # Management Company Information
    memo = models.TextField(blank=True, null=True, help_text="Additional notes or memo about the contract")
    management_company_name = models.CharField(max_length=255, help_text="Name of the management company or person")
    management_company_phone_number = models.CharField(max_length=11, help_text="Management company phone number (maximum 11 digits)")
