...
class Business(TimeStampedUserModel):
    ...
    office_rent = models.DecimalField(
        ...
        help_text="Monthly office rent amount"
    )
...
class MunicipalShop(TimeStampedUserModel):
    ...
    # Rent Details
    shop_rent = models.DecimalField(
        max_digits=10, 
        decimal_places=2, 
        default=0.00,
        validators=[MinValueValidator(0)],
        help_text="Monthly shop rent amount"
    )
...
class Staff(TimeStampedUserModel):
    # Association: staff can belong either to a Business or a Shop, but not both
    business = models.ForeignKey(
        Business, on_delete=models.CASCADE, null=True, blank=True, related_name="staff"
    )
    shop = models.ForeignKey(
        MunicipalShop, on_delete=models.CASCADE, null=True, blank=True, related_name="staff"
    )
    ...
    def __str__(self):
        if self.shop and self.business:
            return f"{self.full_name} ({self.role}) - {self.shop.name} at {self.business.name}"
        elif self.shop:
            return f"{self.full_name} ({self.role}) - {self.shop.name}"
        elif self.business:
            return f"{self.full_name} ({self.role}) - {self.business.name}"
        else:
            return f"{self.full_name} ({self.role})"

    def clean(self):
        # Staff can belong to both business and shop (composite relationship)
        # But if shop is specified, it must belong to the specified business
        if self.shop and self.business:
            if self.shop.business != self.business:
                raise ValidationError("The selected shop must belong to the specified business.")
        elif not self.business and not self.shop:
            raise ValidationError("Staff must belong to either a business or a shop.")
        
        if self.salary < 0:
            raise ValidationError("Salary cannot be negative.")
        
        if self.dob and self.start_date and self.dob > self.start_date:
    ...
    def assignment_display(self):
        """Get a human-readable display of staff assignment"""
        if self.shop and self.business:
    ...
    def terminate_employment(self, end_date=None, reason="Terminated"):
        ...
        # Create salary history record for termination
        # Salary history functionality removed

    def resign_employment(self, end_date=None, reason="Resigned"):
        """Staff resignation"""
        if not end_date:
            end_date = timezone.now().date()
        ...
        # Create salary history record for resignation
        # Salary history functionality removed

    def reactivate_employment(self, new_start_date=None):
        """Reactivate staff employment"""
        ...
        # Create salary history record for reactivation
        # Salary history functionality removed

    # Update salary helper
    def update_salary(self, amount, change_type, reason=""):
        if amount < 0:
            raise ValidationError("Amount cannot be negative.")
            
        previous = self.salary
        ...
        if previous != self.salary:
            change_type = "Increment" if self.salary > previous else "Decrement"
            reason = f"Salary updated from {previous} to {self.salary}"
            # Salary history functionality removed
            pass
...
class Dependent(TimeStampedUserModel):
    ...
    def __str__(self):
        try:
            if self.staff_id and self.staff:
                return f"{self.full_name} ({self.relationship} of {self.staff.full_name})"
            else:
                return f"{self.full_name} (No staff assigned)"
        except Dependent.staff.RelatedObjectDoesNotExist:
            return f"{self.full_name} (No staff assigned)"
    ...
    def clean(self):
        try:
            ...
            # If staff doesn't exist, skip this validation
            pass
