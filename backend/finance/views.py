...
def real_estate_revenue(request):
    ...
    if not (request.user.is_superuser or request.user.has_perm('targets.can_view_real_estate_revenue')):
...
def confirm_ad_fee_receipt(request, pk):
    ...
    if not (request.user.is_superuser or request.user.has_perm('targets.can_confirm_ad_fee')):
        raise PermissionDenied
